...
card.length===25&&
card[12]==="LURA DIES"&&
card.every(item=>typeof item==="string"&&item.trim()!=="")&&
new Set(card.map(item=>item.trim().toLowerCase())).size===25
);
}
...
const setup=new Array(25).fill(null);
setup[12]="LURA DIES";
return setup;
}
...
if(i===12){
setup[i]="LURA DIES";
}else if(typeof saved[i]==="string"&&saved[i].trim()!==""){
setup[i]=saved[i].trim();
}
}
...
const setup=createEmptyChooseSetup();
let position=0;
...
let enabled=loadJSON(STORAGE_KEYS.chooseEnabled,false);
let setup=normalizeChooseSetup(
loadJSON(STORAGE_KEYS.chooseSetup,null)
);

if(!enabled){
saveJSON(STORAGE_KEYS.chooseSetup,setup);
}

function getPlacedCount(){
return setup.filter(
(item,index)=>index!==12&&item
).length;
}
...
bingoItems
.filter(item=>item.trim().toLowerCase()!=="lura dies")
.forEach(item=>{
...
if(count){
count.textContent=`${placed} / 24 Squares Placed`;
}

if(enableButton){
enableButton.disabled=placed!==24||enabled;
enableButton.textContent=enabled?"Card Enabled":"Enable Card";
}
}
...
document.getElementById("enableCardButton")?.addEventListener("click",()=>{
if(getPlacedCount()!==24){
alert("Please place all 24 squares first.");
return;
}

const items=setup.filter(
(item,index)=>index!==12&&item
);

const card=createCardFromItems(items);
...
localStorage.removeItem(STORAGE_KEYS.chooseSetup);
localStorage.removeItem(STORAGE_KEYS.chooseEnabled);
...
alert("Your browser doesn't allow direct image copying, so the card was downloaded instead.");
}catch(error){
console.error(error);
alert("Couldn't create the card image.");
}
